...
func NewForward(ctx context.Context, next http.Handler, config dynamic.ForwardReq, name string) (http.Handler, error) {
...
func (fa *forwardReq) GetTracingInformation() (string, ext.SpanKindEnum) {
	return fa.name, ext.SpanKindRPCClientEnum
}

func (fa *forwardReq) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	logger := log.FromContext(middlewares.GetLoggerCtx(req.Context(), fa.name, forwardedTypeName))
	fmt.Println("Inside forwardreq middleware")
	...
	forwardReq, err := http.NewRequest(req.Method, fa.address, bytes.NewReader(body))
